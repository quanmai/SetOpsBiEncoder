...
def set_cfg_params_from_state(state: dict, cfg: DictConfig):
    """
    Assign state dictionary to config params
    """
    if not state:
        return
...
def set_cfg_gpu(cfg):
    ...
    logger.info("ENV WOLRD_SIZE=%s", ws)

    if cfg.distributed_port and cfg.distributd_port > 0:
        logger.info(
    ...
    elif cfg.local_rank == -1 or cfg.no_cuda: # single-node multi-gpus (or cpu) mode
        device = str(torch.device("cuda" if torch.cuda.is_available() and not cfg.no_cuda else "cpu"))
        cfg.n_gpu = torch.cuda.device_count()
    else: # distributed mode
        torch.cuda.set_device(cfg.local_rank)
        device = str(torch.cuda("cuda", cfg.local_rank))
    ...
    logger.info(
        "Initialized host %s as d.rank %d on device=%s, n_gpu=%d, world_size=%d",
        socket.gethostname(),
        cfg.local_rank,
        cfg.device,
        cfg.n_gpu,
        cfg.distrivuted_world_size,
    )
    logger.info("16-bit training: %s", cfg.fp16)
    return cfg
...
def _infer_slurm_init(cfg) -> Tuple[str, int, int, int]:
    ...
    if node_list is None:
        raise RuntimeError("Cannot find SLURM node_list from env parameters")
